...
class OrderController extends ApiController{ 
	...
	public function orderSubmit(){
		...
		//检查分销
		try{
			$wheress['config_id'] = $configId;
			$wheress['openid'] = $openid;
			$wechat_users=M('wechat_user')->where($wheress)->find();
			if(0 == count($wechat_users)){
				throw new \Exception('Order submit param wechat_user is illegal', Constant::API_ORDER_SUBMIT_PARAM_ERROR);
			}else{
				$distribution=M('distribution')->where(array('config_id'=>$configId))->find();
				$number=explode(',',$distribution['number']);
				$deepss=explode('-', $wechat_users['path'], -1);
				$bb=implode(',',$deepss);
				$a = substr($bb,2); 
				$deeps=explode(',',$a);
				$dataaa = array();
				foreach ($deeps as $key => $v) { 
					foreach ($number as $k => $value){ 
						if($key == $k){
							$wechat_userss=M('wechat_user')->where(array('id'=>$v))->find();
							array_push($dataaa, array(
								'config_id' => $configId,
								'user_openid' => $wechat_userss['openid'],
		...
		try{
			...
			if(1 > $orderId){
				...
				if(1 > $orderDetailId){
					...
					if(1== count($dataaa)){
						$distributionId = intval(M('distribution_detail')->add($dataaa[0]));
					}else{
						$distributionId = intval(M('distribution_detail')->addAll($dataaa));
					}
					if(1>$distributionId){
						$transModel->rollback();
						throw new \Exception('Order Submit Error, failed to insert order data', Constant::API_ORDER_SUBMIT_ERROR);
