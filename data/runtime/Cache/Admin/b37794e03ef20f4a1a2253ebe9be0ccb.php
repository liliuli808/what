...
<body>
	<div class="wrap js-check-wrap">
		<ul class="nav nav-tabs">
			<li class="active"><a href="<?php echo U('Admin/Deliver/C_deliverList'); ?>">水工列表</a></li>
		</ul>
		<form class="well form-search" method="post" action="<?php echo U('Admin/Deliver/C_deliverList'); ?>">
			...
			关键字： 
			<input type="text" name="keyword" style="width: 200px;" value="<?php echo $_POST['keyword']; ?>" placeholder="请输入水工姓名,电话...">
 ...
 if($v['status'] == 1) echo '启用'; if($v['status'] == 0) echo '禁用'; ?>
							...
							<td>
								<a href="<?php echo U('Admin/Deliver/C_editDeliver', array('did'=>$v['id'])); ?>">编辑</a>
								<?php
 if(0 == $v['status']){ echo ' | <a class="js-ajax-dialog-btn" data-msg="您确定要启用此水工么？" href="'.U('Admin/Deliver/C_openDeliver', array('cid'=>$v['id'])).'">启用</a>'; } ?>
								<?php
 if(1 == $v['status']){ echo ' | <a class="js-ajax-dialog-btn" data-msg="您确定要禁用此水工么？" href="'.U('Admin/Deliver/C_closeDeliver', array('cid'=>$v['id'])).'">禁用</a>'; } ?>
								<!-- 
								<?php
 ...
 if(0 == $v['is_recommended']){ echo ' |<a class="js-ajax-dialog-btn" data-msg="您确定要推荐此商品至首页么？" href="'.U('Admin/Goods/C_recommendGoods', array('gid'=>$v['id'])).'">推荐</a>'; } ?>
								 -->
								 | <a class="js-ajax-dialog-btn" data-msg="您确定要删除此水工么？" href="<?php echo U('Admin/Deliver/C_deleteDeliver', array('cid'=>$v['id'])); ?>">删除</a>
							</td>
						</tr>
					<?php } ?>
				</tbody>
